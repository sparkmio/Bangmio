...
export default async function CharacterPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <><div className="detail-hero panel"><div className="detail-cover cover-placeholder">{character.name?.slice(0, 1) || '人'}</div><div className="detail-content"><div className="eyebrow">Character</div><h1>{character.name_cn || character.name}</h1><p className="detail-original">{character.name && character.name_cn ? character.name : '角色资料'}</p><div className="detail-summary"><RichText value={character.info || character.summary} fallback="暂无简介。" /></div></div></div><section><h2>出演作品</h2><div className="list-grid">{list.map((item: any) => <Link className="list-card" href={`/anime/${item.id || item.subject_id}`} key={item.id || item.subject_id}><img className="list-cover" src={imageUrl(item.images) || ''} alt="" /><div><h3>{item.name_cn || item.name || '未命名作品'}</h3><p>{item.role_name || '出演角色'}</p></div></Link>)}</div></section></>
}
