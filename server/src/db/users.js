 ...
 *   users(id, email, password_hash, salt, session_version, bgm_uid,
 *         bgm_token_encrypted, bgm_token_iv, created_at, updated_at)
 *
...
export async function createUser(db, { id, email, passwordHash, salt }) {
  ...
  try {
    const result = await db
      ...
      .bind(id, normalizedEmail, passwordHash, salt, now, now)
      .run()
    if (!result.success) {
 ...
 */
export async function getUserSessionVersion(db, id) {
  try {
    const row = await db
      .prepare('SELECT session_version AS sessionVersion FROM users WHERE id = ?')
      .bind(id)
      .first()
