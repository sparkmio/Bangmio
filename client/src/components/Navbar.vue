<template>
  <div class="md:hidden sticky top-0 z-50 bg-base-100/80 backdrop-blur-xl border-b border-base-300">
    <div class="flex items-center justify-between px-4 h-14">
      ...
      <div class="flex items-center gap-1">
        <button @click="theme.toggle()" class="btn btn-ghost btn-sm btn-circle text-base-content/50 hover:text-violet-500">
        ...
        <template v-if="auth.isLoggedIn">
          <router-link to="/profile" class="btn btn-ghost btn-sm btn-circle avatar p-0.5">
            <div class="w-7 h-7 rounded-full ring-1 ring-violet-500/30">
              ...
              <div v-else class="w-7 h-7 rounded-full bg-violet-500 text-white flex items-center justify-center text-[10px] font-bold serif-cn">
  ...
  <div class="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-base-100/80 backdrop-blur-xl border-t border-base-300">
    <div class="grid grid-cols-4 h-14">
      <router-link to="/" :class="$route.path === '/' ? 'text-violet-500' : 'text-base-content/40'" class="flex flex-col items-center justify-center gap-0.5 transition-colors">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
        <span class="text-[10px] font-medium serif-cn">首页</span>
      </router-link>
      <router-link to="/anime" :class="$route.path === '/anime' ? 'text-violet-500' : 'text-base-content/40'" class="flex flex-col items-center justify-center gap-0.5 transition-colors">
      ...
      <router-link to="/settings" :class="$route.path === '/settings' ? 'text-violet-500' : 'text-base-content/40'" class="flex flex-col items-center justify-center gap-0.5 transition-colors">
      ...
      <router-link to="/profile" :class="$route.path === '/profile' ? 'text-violet-500' : 'text-base-content/40'" class="flex flex-col items-center justify-center gap-0.5 transition-colors">
