...
export default async function SubjectTopicsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="subject-discussion-page">
    ...
    {topics.length ? <section className="topic-list panel" aria-label="条目话题列表">{topics.map((topic: any, index: number) => <Link className="topic-row" href={`/topic/${topic.id}`} key={topic.id || index}><span className="topic-avatar">话</span><span className="topic-row-copy"><strong>{safeText(topic.title ?? topic.name, '未命名话题')}</strong><small>{safeText(topic.creator?.nickname ?? topic.creator?.username, '社区成员')} · {countValue(topic.replies ?? topic.reply_count)} 条回复</small></span><span className="topic-row-arrow" aria-hidden="true">→</span></Link>)}</section> : <div className="panel empty-state"><div className="empty-icon">✦</div><h3>还没有话题</h3><p>成为第一个开始讨论的人。</p></div>}
  </div>
}
