...
export function AuthForm({ mode }: { mode: Mode }) {
  ...
  return <div className="panel form-card"><div className="eyebrow">{currentMode === 'register' ? 'Create account' : isBangumi ? 'Bangumi access' : 'Welcome back'}</div><h1>{title}</h1><p>{currentMode === 'register' ? '验证邮箱后创建账号，再把收藏同步到 Bangumi。' : currentMode === 'bind' ? '绑定后，收藏、进度和个人页会使用你的 Bangumi 数据。' : isBangumi ? 'Token 仅保存在当前浏览器，用于读取并管理 Bangumi 数据。' : '登录后继续管理收藏、观看进度和社区内容。'}</p><form className="form-stack" onSubmit={submit}>{!isBangumi ? <><label>邮箱<input className="input" type="email" value={email} onChange={event => setEmail(event.target.value)} required placeholder="you@example.com" /></label><label>密码<input className="input" type="password" value={password} onChange={event => setPassword(event.target.value)} required minLength={8} placeholder="至少 8 位" /></label>{currentMode === 'register' ? <label>邮箱验证码<div className="inline-field"><input className="input" value={code} onChange={event => setCode(event.target.value)} required placeholder="6 位验证码" /><button className="button ghost compact" type="button" disabled={busy || seconds > 0} onClick={sendCode}>{seconds > 0 ? `${seconds}s 后重发` : codeSent ? '重新发送' : '发送验证码'}</button></div></label> : null}</> : <label>Bangumi Access Token<textarea className="input" value={bangumiToken} onChange={event => setBangumiToken(event.target.value)} required rows={4} placeholder="粘贴 Bangumi Access Token" /></label>}<button className="button primary" disabled={busy} type="submit">{busy ? '处理中…' : currentMode === 'register' ? '注册并继续' : currentMode === 'bind' ? '完成绑定' : '继续'}</button>{message ? <div className="form-message">{message}</div> : null}</form>{currentMode === 'login' ? <div className="form-footer">还没有账号？<Link href="/register">去注册</Link><br /><button className="text-button" disabled={busy} onClick={() => openOAuth('login')} type="button">使用 Bangumi OAuth 登录</button><br /><Link href="/login?mode=bangumi">使用 Access Token 登录</Link><br /><Link href="/forgot-password">忘记密码？</Link></div> : currentMode === 'bind' ? <div className="form-footer"><button className="text-button" disabled={busy} onClick={() => openOAuth('bind')} type="button">使用 Bangumi OAuth 绑定</button><br /><Link href="/profile">暂时跳过</Link></div> : currentMode === 'register' ? <div className="form-footer">已有账号？<Link href="/login">返回登录</Link></div> : null}</div>
}
