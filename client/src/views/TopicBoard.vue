<template>
  <div class="max-w-4xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
      <router-link :to="`/anime/${route.params.id}`" class="btn btn-ghost btn-sm text-primary">← 返回</router-link>
    ...
    <div v-else-if="error" class="text-center py-20">
      ...
      <button @click="fetchTopics" class="btn btn-ghost btn-sm text-primary">重试</button>
    </div>
      ...
      >
        <div class="card-body p-4 flex-row items-center justify-between">
          ...
          <div class="flex-shrink-0 ml-4">
            <span class="badge badge-sm" :class="topic.replies > 0 ? 'badge-primary' : 'badge-ghost'">
              {{ topic.replies }} 回复
            </span>
          </div>
        </div>
