<template>
  <router-link :to="`/anime/${anime.id}`" class="group block" ref="cardRef">
    <div class="relative overflow-hidden rounded-md bg-base-300 paper-shadow group-hover:-translate-y-1.5">
      <div class="aspect-[2/3] relative">
        ...
        <div v-else class="w-full h-full flex items-center justify-center bg-base-300">
          <span class="text-[10px] text-base-content/30 serif-cn tracking-wider">暂无封面</span>
        </div>
        ...
        <div v-if="anime.rating?.score" class="absolute top-2 left-2 z-10">
          <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold bg-black/80 text-amber-400 numeric">
        ...
        <div v-if="anime.rank" class="absolute top-2 right-2 z-10">
          <span class="px-1.5 py-0.5 rounded text-[10px] font-bold bg-black/80 text-white numeric">#{{ anime.rank }}</span>
        </div>

        <!-- 标题 -->
        <div class="absolute inset-x-0 bottom-0 p-3 z-10">
          <h3 class="text-[13px] font-semibold text-white line-clamp-2 leading-snug serif-cn">{{ anime.name_cn || anime.name }}</h3>
          <p v-if="anime.name_cn && anime.name" class="text-[10px] text-white/60 mt-0.5 line-clamp-1 font-mono tracking-tight">{{ anime.name }}</p>
        </div>
      </div>
    </div>
  </router-link>
