...
export default async function PersonPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    <div className="detail-hero panel">
      ...
      <div className="detail-content"><div className="eyebrow">Person #{id}</div><h1>{person.name_cn || person.name}</h1><p className="detail-original">{person.name && person.name_cn ? person.name : '人物资料'}</p><div className="detail-summary"><RichText value={person.summary} fallback="暂无简介。" /></div><div className="detail-actions"><Link className="button primary" href={`/person/${id}/talkbox`}>人物吐槽箱</Link></div></div>
    </div>
