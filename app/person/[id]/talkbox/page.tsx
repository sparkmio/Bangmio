  ...
  return (
    <>
      <div className="section-heading">
        <div>
          <div className="eyebrow">Person Talkbox</div>
          <h1>人物吐槽箱</h1>
          <p>围绕这位人物留下短评。</p>
        </div>
        <Link className="button ghost" href={`/person/${id}`}>
          返回人物
        </Link>
      </div>
      ...
      {!response ? (
        ...
        <div className="list-grid">
          {comments.map((comment: any, index: number) => {
            ...
            return (
              <div className="list-card" key={comment.id || index}>
                ...
                <div>
                  <h3>{user.name}</h3>
                  <RichText value={content} fallback="暂无内容。" />
                  {comment.timestamp ? (
                    <small className="muted-copy">{comment.timestamp}</small>
                  ) : null}
                </div>
              </div>
            )
