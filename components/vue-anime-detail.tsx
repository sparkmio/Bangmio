...
function importantInfobox(infobox: InfoboxItem[]) {
  ...
  return infobox.filter(item => important.test(String(item.key || ''))).slice(0, 12)
}
...
export function VueAnimeDetail({ subject, relations, characters, persons, episodes, infobox }: Props) {
  ...
  return <div>
    <div className="-mx-4 md:-mx-8 -mt-6 relative overflow-hidden"><div className="absolute inset-0 overflow-hidden">{image ? <img src={image} alt="" className="w-full h-full object-cover scale-110 blur-3xl opacity-30" /> : null}<div className="absolute inset-0 bg-gradient-to-b from-base-100/60 via-base-100/90 to-base-100" /></div><div className="relative max-w-5xl mx-auto px-4 md:px-8 py-10 md:py-16"><Link href="/anime" className="btn btn-ghost btn-sm text-primary/80 mb-4">← 返回</Link><div className="flex flex-col md:flex-row gap-6 md:gap-8 items-center md:items-start"><div className="w-40 sm:w-48 md:w-60 shrink-0">{image ? <img src={image} alt={title} className="w-full rounded-2xl shadow-2xl ring-1 ring-white/10" loading="eager" decoding="async" /> : <div className="w-full aspect-[2/3] rounded-2xl bg-base-300 flex items-center justify-center">暂无封面</div>}</div><div className="flex-1 min-w-0 text-center md:text-left"><h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-2 break-words">{title}</h1>{subject.name_cn && subject.name && subject.name_cn !== subject.name ? <p className="text-base text-base-content/50 mb-4">{subject.name}</p> : null}<div className="flex flex-wrap items-center justify-center md:justify-start gap-2 mb-6">{subject.rating?.score ? <span className="badge badge-lg gap-1.5 font-bold border-0 bg-amber-500/15 text-amber-400">★ {Number(subject.rating.score).toFixed(1)}<span className="text-xs font-normal text-base-content/40">({subject.rating.total || 0}人)</span></span> : null}{subject.rating?.rank ? <span className="badge badge-lg font-bold border-0 bg-primary/15 text-primary">#{subject.rating.rank}</span> : null}<span className="badge badge-lg badge-ghost">{typeLabel}</span>{subject.eps || subject.eps_count ? <span className="badge badge-lg badge-ghost">{subject.eps || subject.eps_count}话</span> : null}</div><CollectionEditor animeId={subject.id} /></div></div></div></div>
    <div className="sticky top-0 z-30 bg-base-100/80 backdrop-blur-md border-b border-base-300/50"><div className="max-w-5xl mx-auto px-4 md:px-8 flex gap-1 overflow-x-auto scrollbar-hide">{tabs.map(([key, label]) => <button key={key} className={`px-3 py-3 min-h-[44px] text-sm font-medium whitespace-nowrap border-b-2 transition-colors ${activeTab === key ? 'border-primary text-primary' : 'border-transparent text-base-content/60 hover:text-base-content'}`} type="button" onClick={() => setActiveTab(key)}>{label}</button>)}</div></div>
    <div className="max-w-5xl mx-auto px-4 md:px-8 py-6">
      {activeTab === 'overview' ? <div className="space-y-7"><section><SectionTitle>简介</SectionTitle><p className="text-sm leading-relaxed text-base-content/70 whitespace-pre-line">{subject.summary || '暂无简介。'}</p>{subject.tags?.length ? <div className="flex flex-wrap gap-1.5 mt-4">{subject.tags.slice(0, 12).map(tag => <span key={tag.name} className="badge badge-sm badge-ghost">{tag.name}</span>)}</div> : null}</section><section><SectionTitle>评分与收藏</SectionTitle><div className="grid grid-cols-1 md:grid-cols-2 gap-5"><RatingChart subject={subject} /><CollectionChart subject={subject} /></div></section><section><SectionTitle>基本信息</SectionTitle><div className="grid grid-cols-2 sm:grid-cols-4 gap-3">{[['类型', typeLabel], ['首播', subject.air_date || subject.date || '未知'], ['集数', subject.eps || subject.eps_count || '未知'], ['排名', subject.rating?.rank ? `#${subject.rating.rank}` : '暂无']].map(([label, value]) => <div key={label} className="rounded-lg bg-base-200/40 p-3"><p className="text-xs text-base-content/50">{label}</p><p className="text-sm mt-1">{value}</p></div>)}</div></section>{infoboxItems.length ? <section><SectionTitle>制作信息</SectionTitle><div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{infoboxItems.map((item, index) => <div key={`${item.key}-${index}`} className="rounded-lg bg-base-200/40 p-3"><p className="text-xs text-base-content/50">{item.key}</p><p className="text-sm mt-1">{valueText(item.value)}</p></div>)}</div></section> : null}<ExternalLinks subject={subject} /></div>
