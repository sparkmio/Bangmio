<template>
  <div>
    ...
    <div v-if="!loading && !error && person.id" class="max-w-4xl mx-auto">
      ...
      <div class="flex flex-col md:flex-row gap-8 mb-8">
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-sm mb-3 text-base-content/50">
            ...
            <span v-if="person.gender" class="ml-2">
              ·
              {{
                person.gender === 'male'
                  ? '♂ 男'
            ...
            >
            <span v-if="person.blood_type" class="ml-2">
              · {{ ['', 'A', 'B', 'AB', 'O'][person.blood_type] }}型血</span
            >
          </p>
          <p v-if="person.birth_year" class="text-sm mb-3 text-base-content/50">
            {{ person.birth_year
            }}{{ person.birth_mon ? '-' + String(person.birth_mon).padStart(2, '0') : ''
            }}{{ person.birth_day ? '-' + String(person.birth_day).padStart(2, '0') : '' }}
          </p>
      ...
      <router-link
        :to="`/person/${person.id}/talkbox`"
        class="btn btn-outline btn-sm w-full border-base-300"
      >
        查看吐槽箱 →
      </router-link>
    </div>
