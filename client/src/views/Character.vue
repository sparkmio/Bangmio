    ...
    <div v-if="!loading && !error && character.id" class="max-w-4xl mx-auto">
      <div class="flex flex-col md:flex-row gap-8 mb-8">
        ...
        <div class="flex-1 min-w-0">
          <h1 class="text-2xl md:text-3xl font-bold mb-1 text-base-content">{{ character.name }}</h1>
          <p v-if="character.gender || character.birth_year || character.type" class="text-sm mb-3 text-base-content/50">
            {{ chrTypeLabel }}
          ...
          <div v-if="character.summary" class="mb-4">
            ...
            <p class="text-sm leading-relaxed text-base-content/70">{{ character.summary }}</p>
          </div>

          <div v-if="character.stat" class="flex gap-6 mb-4">
            <div class="text-center">
              <p class="text-xl font-bold text-amber-500">{{ character.stat.comments || 0 }}</p>
              <p class="text-xs text-base-content/50">评论</p>
            </div>
            <div class="text-center">
              <p class="text-xl font-bold text-primary">{{ character.stat.collects || 0 }}</p>
      ...
      <div v-if="chrPersons.length" class="mb-8">
        ...
        <div class="flex flex-wrap gap-4">
          <div v-for="cp in chrPersons" :key="cp.id" class="flex items-center gap-3 p-3 rounded-xl bg-base-100 border border-base-300 hover:border-primary transition-all">
            <div class="avatar placeholder">
              <div class="w-10 h-10 rounded-full bg-primary text-primary-content">
                <img v-if="cp.images?.medium || cp.images?.grid" :src="cp.images.medium || cp.images.grid" />
                <span v-else class="text-xs">{{ cp.name?.[0] }}</span>
              </div>
            </div>
            <div>
              <p class="text-sm font-medium text-base-content">{{ cp.name }}</p>
              <p class="text-xs text-base-content/50">{{ cp.staff || cp.subject_name_cn || cp.subject_name }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="chrSubjects.length" class="mb-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          <router-link v-for="subj in chrSubjects" :key="subj.id" :to="`/anime/${subj.id}`" class="card card-side bg-base-100 border border-base-300 hover:border-primary transition-all">
            ...
            <div class="card-body p-3 py-2">
              ...
              <p class="text-xs text-base-content/50">{{ subj.staff || typeLabel(subj.type) }}</p>
            </div>
          </router-link>
        </div>
      </div>
      ...
      <div class="flex gap-2 mb-8">
        <a :href="`https://bgm.tv/character/${character.id}`" target="_blank" class="btn btn-ghost btn-sm text-base-content/60">
          在 Bangumi 查看完整信息 →
        </a>
      </div>
