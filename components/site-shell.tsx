...
type IconName =
  'home' | 'trending' | 'browse' | 'groups' | 'about' | 'settings' | 'moon' | 'sun' | 'user'
...
function ProfileAvatar({ size = 'w-8 h-8' }: { size?: string }) {
  ...
  return (
    <div className="avatar">
      <div className={`${size} rounded-full ring-2 ring-primary/20`}>
        {avatar ? (
          <img src={avatar} alt={name} className="rounded-full" decoding="async" />
        ) : (
          <div
            className={`${size} rounded-full bg-primary text-primary-content flex items-center justify-center text-sm font-bold`}
          >
...
export function SiteShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const protectedRoute =
      pathname === '/watching' || pathname === '/profile' || pathname === '/settings'
  ...
  return (
    <div className="min-h-screen bg-base-100">
        ...
        <div className="flex items-center gap-3 px-5 h-16 border-b border-base-300/30 shrink-0">
          <Link href="/" className="flex items-center gap-3">
            <img
              src="/logo.png"
              alt="Bangmio"
              className="w-8 h-8 rounded-xl ring-2 ring-primary/20"
              decoding="async"
            />
            <span className="text-lg font-bold tracking-tight text-primary">Bangmio</span>
          </Link>
        </div>
        <ul className="menu flex-1 py-4 px-3 gap-1">
          {navItems.map(item => (
            <li key={item.href}>
              <Link
                href={item.href}
                className={`gap-3 rounded-lg ${isActive(pathname, item.href) ? 'active' : ''}`}
              >
                <Icon name={item.icon} />
        ...
        <div className="px-3 py-4 border-t border-base-300/30 shrink-0">
          {isAuthenticated ? (
            <>
              <Link
                href="/profile"
                className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-primary/5 hover:bg-primary/10 transition-colors"
              >
                <ProfileAvatar />
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium truncate text-base-content">{accountName}</p>
                  <p className="text-xs truncate text-base-content/40">查看主页</p>
                </div>
              </Link>
              <button
                className="btn btn-ghost btn-xs w-full mt-2 text-base-content/40 hover:text-error rounded-full"
                type="button"
                onClick={logout}
              >
                退出登录
            ...
            <Link
              href="/login"
              className="btn btn-primary btn-sm w-full rounded-full shadow-lg shadow-primary/20"
            >
              登录 Bangmio
            </Link>
          )}
          <button
            className="btn btn-ghost btn-xs w-full mt-3 gap-2 rounded-full"
            type="button"
            onClick={toggleTheme}
          >
      ...
      <div className="md:hidden sticky top-0 z-50 navbar bg-base-100/60 backdrop-blur-xl border-b border-base-300/50">
        <div className="navbar-start">
          <Link href="/" className="flex items-center gap-2">
            <img src="/logo.png" alt="Bangmio" className="w-7 h-7 rounded-lg" decoding="async" />
            <span className="text-lg font-black text-primary">Bangmio</span>
          </Link>
        </div>
      ...
      <nav
        className="md:hidden fixed bottom-0 left-0 right-0 z-50 btm-nav bg-base-100/60 backdrop-blur-xl border-t border-base-300/50 min-h-[3.5rem]"
      ...
      >
        {(
          [
            ...
            ['/profile', isAuthenticated ? '我的' : '登录', 'user']
          ] as Array<[string, string, IconName]>
        ).map(([href, label, icon]) => (
          <Link
            key={href}
            href={href}
            className={`min-h-[44px] ${isActive(pathname, href) ? 'active text-primary' : 'text-base-content/40'}`}
          >
            <Icon name={icon} />
