...
import { pageContext } from '@/lib/ai-context'
...
function createConversationId() {
  ...
  if (webCrypto)
    return `${Date.now()}-${Array.from(webCrypto.getRandomValues(new Uint32Array(2))).join('-')}`
...
function inlineMarkdown(value: string): ReactNode[] {
  ...
  for (const match of value.matchAll(pattern)) {
    ...
    if (token.startsWith('[')) {
      ...
      if (parsed && /^https?:\/\//i.test(parsed[2]))
        tokens.push(
          <a key={`${index}-link`} href={parsed[2]} target="_blank" rel="noopener noreferrer">
            {parsed[1]}
          </a>
        )
      ...
      tokens.push(
        <a key={`${index}-url`} href={href} target="_blank" rel="noopener noreferrer">
          {token}
        </a>
      )
    } else if (token.startsWith('`'))
      tokens.push(<code key={`${index}-code`}>{token.slice(1, -1)}</code>)
    else if (token.startsWith('~~'))
      tokens.push(<del key={`${index}-del`}>{token.slice(2, -2)}</del>)
    else if (token.startsWith('**') || token.startsWith('__'))
      tokens.push(<strong key={`${index}-strong`}>{token.slice(2, -2)}</strong>)
...
function MarkdownMessage({ content }: { content: string }) {
    ...
    blocks.push(
      <blockquote key={`quote-${blocks.length}`}>
        {quote.map((item, index) => (
          <p key={index}>{inlineMarkdown(item)}</p>
        ))}
      </blockquote>
    )
    quote = []
  ...
  lines.forEach((line, index) => {
    ...
    if (heading) {
      ...
      blocks.push(
        createElement(headingTag, { key: `heading-${index}` }, inlineMarkdown(heading[2]))
      )
...
export function AiChat({ context }: { context?: string }) {
  ...
  const messages = useMemo(() => active?.messages || [OPENING], [active?.messages])
  const contextAllowed = /^\/(anime|person|character)\/\d+\/?$/.test(pathname)
  useEffect(() => {
  ...
  const createConversation = useCallback(() => {
    ...
    setConversations(items => {
      const updated = [next, ...items.filter(item => item.id !== next.id)]
  ...
  async function submit(event: FormEvent) {
    ...
    const nextMessages = [
      ...conversation.messages.filter(message => !isOpening(message)),
      { role: 'user' as const, content }
    ].slice(-12)
    const optimistic = {
      ...conversation,
      title: conversation.title === '新对话' ? content.slice(0, 24) : conversation.title,
      messages: nextMessages,
      updatedAt: Date.now()
    }
    setConversations(items => {
      const updated = [optimistic, ...items.filter(item => item.id !== conversation.id)]
      saveConversations(updated)
      return updated
    })
    setInput('')
    ...
    try {
      const response = await fetch('/api/v1/ai/chat', {
        ...
        body: JSON.stringify({
          messages: nextMessages,
          context: includeContext && contextAllowed ? pageContext(context) : ''
        })
      })
      ...
      const completed = {
        ...
        messages: [
          ...nextMessages,
          { role: 'assistant' as const, content: payload.data.message }
        ].slice(-12),
        updatedAt: Date.now()
      }
      setConversations(items => {
        const updated = [completed, ...items.filter(item => item.id !== conversation.id)]
        saveConversations(updated)
        return updated
      })
    } catch (cause) {
  ...
  return (
    <>
      {open ? (
        <section className="bm-ai-panel" role="dialog" aria-label="Bangmio AI 对话">
          <header className="bm-ai-header">
            <div>
              <h2>Bangmio AI</h2>
              <p>澪 · 当前页面资料助手</p>
            </div>
            <div className="bm-ai-header-actions">
              <button
                type="button"
              ...
              <button type="button" onClick={createConversation} aria-label="新建对话">
                新建
              </button>
              <button type="button" onClick={() => setOpen(false)} aria-label="关闭 AI 对话">
                ×
              </button>
            </div>
          </header>
          {historyOpen ? (
            <aside className="bm-ai-history">
              <div className="bm-ai-history-title">历史对话</div>
          ...
          <div ref={scrollRef} className="bm-ai-messages">
            {messages.map((message, index) => (
              <div
                ...
                className={`bm-ai-message ${message.role === 'user' ? 'is-user' : 'is-assistant'}`}
              >
                {message.role === 'assistant' ? (
                  <MarkdownMessage content={message.content} />
                ) : (
                  <RichText value={message.content} fallback="" />
                )}
              </div>
            ))}
            {busy ? (
              <div className="bm-ai-message is-assistant">
                <p>正在思考…</p>
              </div>
            ) : null}
          </div>
          <div className="bm-ai-suggestions">
          ...
          <form className="bm-ai-form" onSubmit={submit}>
            <p className="text-xs mb-2">
              发送的问题将交由第三方 AI 服务处理。不会自动读取账号页面或发送收藏记录。
            </p>
            {contextAllowed ? (
              <label className="text-xs flex gap-2 mb-2">
                <input
                  type="checkbox"
