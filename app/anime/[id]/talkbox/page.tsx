...
export default async function SubjectTalkboxPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="subject-discussion-page">
    ...
    {comments.length ? <section className="comment-thread" aria-label="吐槽列表">{comments.map((comment: any, index: number) => {
      ...
      return <article className="panel comment-card" key={comment.id || index}><span className="avatar">{profile.avatar ? <img src={profile.avatar} alt="" loading="lazy" /> : profile.name.slice(0, 1)}</span><div><div className="comment-card-head"><strong>{profile.name}</strong><small>{commentDate(comment, index)}</small></div><RichText value={comment.content || comment.comment} fallback="暂无内容。" /></div></article>
