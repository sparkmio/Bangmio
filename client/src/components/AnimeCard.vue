<template>
  <router-link :to="`/anime/${anime.id}`" class="group block" ref="cardRef">
    <div class="card card-compact bg-base-100 shadow-md hover:shadow-xl transition-all duration-300 group-hover:-translate-y-1 border border-base-300">
      <figure class="relative aspect-[3/4]">
        ...
        <div v-else class="w-full h-full flex items-center justify-center bg-base-200">
          <span class="text-xs text-base-content/40">暂无封面</span>
        </div>
        <div v-if="anime.rating?.score" class="absolute top-2 left-2 z-10">
          <span class="badge badge-sm gap-1 font-bold bg-black/60 text-amber-400 border-amber-400/20 backdrop-blur-sm">
        ...
        <div v-if="anime.rank" class="absolute top-2 right-2 z-10">
          <span class="badge badge-sm font-bold bg-black/60 text-white border-white/10 backdrop-blur-sm">#{{ anime.rank }}</span>
        </div>
        <div class="absolute inset-x-0 bottom-0 p-3 pt-16 bg-gradient-to-t from-black/80 via-black/30 to-transparent">
