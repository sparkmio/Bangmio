...
export function CollectionEditor({ animeId }: { animeId: number }) {
  ...
  return <section className="panel collection-editor"><div><div className="eyebrow">My collection</div><h2>我的收藏</h2><p>状态值已严格对应 Bangumi：<b>看过 = 2</b>，<b>在看 = 3</b>。</p></div><div className="collection-editor-actions"><CollectionButton animeId={animeId} initialStatus={Number(collection?.type || 0)} onSaved={next => setCollection(next)} /></div><form className="form-stack collection-details" onSubmit={saveDetails}><label>评分（0 为未评分）<select className="input" value={rating} onChange={event => setRating(Number(event.target.value))}>{Array.from({ length: 11 }, (_, value) => <option value={value} key={value}>{value === 0 ? '未评分' : `${value} 分`}</option>)}</select></label><label>短评<textarea className="input" rows={3} maxLength={2000} value={comment} onChange={event => setComment(event.target.value)} placeholder="写点观后感…" /></label><button className="button ghost" type="submit" disabled={busy}>{busy ? '保存中…' : '保存评分与短评'}</button></form>{message ? <p className="action-message">{message}</p> : null}</section>
}
