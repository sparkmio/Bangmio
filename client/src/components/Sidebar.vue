<template>
  <aside
    class="hidden md:flex flex-col fixed left-0 top-0 bottom-0 w-56 z-40 bg-base-100/40 backdrop-blur-xl border-r border-base-300/30"
  >
    <div class="flex items-center gap-3 px-5 h-16 border-b border-base-300/30 shrink-0">
      <router-link to="/" class="flex items-center gap-3">
        <img
          src="/logo.png"
          alt="Bangmio"
          class="w-8 h-8 rounded-xl ring-2 ring-primary/20"
          decoding="async"
        />
    ...
    <div class="px-3 py-4 border-t border-base-300/30 shrink-0">
      <template v-if="auth.isLoggedIn">
        <router-link
          to="/profile"
          class="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-primary/5 hover:bg-primary/10 transition-colors"
        >
          <div class="avatar">
        ...
        <button
          class="btn btn-ghost btn-xs w-full mt-2 text-base-content/40 hover:text-error rounded-full"
          @click="auth.logout()"
        >
          退出登录
        </button>
      ...
      <template v-else>
        <router-link
          to="/login"
          class="btn btn-primary btn-sm w-full rounded-full shadow-lg shadow-primary/20"
        >
          登录 Bangmio
        </router-link>
      </template>

      <button class="btn btn-ghost btn-xs w-full mt-3 gap-2 rounded-full" @click="theme.toggle()">
        <svg
...
const IconGame = {
  template:
    '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h8m-4-4v8m-5.5-9h11A3.5 3.5 0 0 1 21 10.5v3a3.5 3.5 0 0 1-3.5 3.5l-2.2-2H8.7l-2.2 2A3.5 3.5 0 0 1 3 13.5v-3A3.5 3.5 0 0 1 6.5 7Z"/></svg>'
}
const IconAbout = {
  template:
...
const navItems = [
  ...
  { path: '/jump', label: '跳一跳', icon: IconGame },
