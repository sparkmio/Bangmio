    ...
    <div class="relative overflow-hidden rounded-lg bg-base-300 card-lift">
      <div class="aspect-[2/3] relative overflow-hidden">
        <img
          ...
          class="w-full h-full object-cover transition-all duration-500 group-hover:brightness-110 group-hover:scale-[1.03]"
          loading="lazy"
          @error="imgError = true"
        />
        ...
        <div class="absolute inset-0 bg-gradient-to-t from-black/60 via-black/10 to-transparent transition-all duration-300 group-hover:from-black/75"></div>
        ...
        <div v-if="anime.rank" class="absolute top-2 right-2 z-10">
          <span class="px-1.5 py-0.5 rounded text-[11px] font-bold bg-black/60 text-white backdrop-blur-sm">#{{ anime.rank }}</span>
        </div>

        <!-- Title at bottom -->
        <div class="absolute inset-x-0 bottom-0 p-3 z-10">
          <h3 class="text-[13px] font-semibold text-white line-clamp-2 leading-snug drop-shadow-sm">{{ anime.name_cn || anime.name }}</h3>
