<template>
  <div class="max-w-4xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
      <a class="btn btn-ghost btn-sm text-primary cursor-pointer" @click.prevent="$router.back()"
        >← 返回</a
      >
      <h1 class="text-lg font-bold truncate text-base-content">
        {{ topic.op?.title || '帖子详情' }}
      </h1>
      <a
        :href="`https://bgm.tv/subject/topic/${route.params.id}`"
        target="_blank"
    ...
    <div v-else-if="error" class="text-center py-20">
      ...
      <button class="btn btn-ghost btn-sm text-primary" @click="fetchTopic">重试</button>
    </div>
    ...
    <div v-else>
      <div v-if="topic.op" class="card bg-base-100 border border-base-300 mb-6">
        <div class="card-body p-5">
          <div class="flex items-start gap-3 mb-3">
            ...
            <div>
              <a
                :href="`https://bgm.tv${topic.op.user?.url}`"
                target="_blank"
                class="text-sm font-medium hover:underline text-base-content"
                >{{ topic.op.user?.username }}</a
              >
              <span class="text-xs ml-2 text-base-content/50">{{ topic.op.timestamp }}</span>
            </div>
          </div>
          <div class="text-sm leading-relaxed whitespace-pre-wrap break-words text-base-content/70">
            {{ topic.op.content }}
          </div>
        </div>
      </div>
      ...
      <div v-if="topic.replies?.length" class="mb-4">
        <h2 class="text-sm font-bold mb-3 text-base-content/50">
          回复 ({{ topic.replies.length }})
        </h2>
        <div class="space-y-3">
          <div
            v-for="r in topic.replies"
            :key="r.id"
            ...
            <div class="card-body p-4">
              <div class="flex items-start gap-3">
                ...
                <div class="min-w-0 flex-1">
                  ...
                  <p
                    class="text-sm leading-relaxed whitespace-pre-wrap break-words text-base-content/70"
                  >
                    {{ r.content }}
                  </p>
                      ...
                      <div class="min-w-0 flex-1">
                        <div class="flex items-center gap-1.5 mb-0.5">
                          <a
                            ...
                            target="_blank"
                            class="text-xs font-medium hover:underline text-base-content"
                            >{{ sub.user?.username }}</a
                          >
